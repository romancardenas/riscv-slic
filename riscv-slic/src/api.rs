...
unsafe extern "Rust" {
    ...
    fn __riscv_slic_raise_threshold(priority: u8) -> Result<u8, u8>;
...
pub unsafe fn set_threshold(priority: u8) {
    unsafe { __riscv_slic_set_threshold(priority) };
}

/// Stabilized API for reading the priority of a software interrupt of the SLIC.
#[inline]
...
pub unsafe fn set_priority<I: crate::InterruptNumber>(interrupt: I, priority: u8) {
    unsafe { __riscv_slic_set_priority(interrupt.number(), priority) };
}
